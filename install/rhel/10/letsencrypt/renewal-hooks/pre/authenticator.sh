...
if [[ -n "${CERTBOT_DOMAIN}" ]] ; then
  ...
  if [[ ! -f "/home/${CERTBOT_USER}/conf/web/${CERTBOT_DOMAIN_BASE}.nginx.ssl.conf" ]] ; then
      sed -i '/return 301 https:/d' "/home/${CERTBOT_USER}/conf/web/${CERTBOT_DOMAIN_BASE}.nginx.conf"
  fi
