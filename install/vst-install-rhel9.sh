...
software="nginx awstats bash-completion bc bind bind-libs bind-utils clamav-server
    clamav-update curl dovecot e2fsprogs exim expect fail2ban flex freetype ftp 
    GeoIP httpd ImageMagick jwhois libidn lsof mariadb 
    mariadb-server mc mod_fcgid mod_ssl net-tools openssh-clients pcre 
    ...
    vesta-php vesta-softaculous vim vsftpd which zip"
...
if [ ! -z "$(grep ^NAME=\"Red /etc/os-release)" ]; then
    ...
    yum install https://dl.fedoraproject.org/pub/epel/epel{,-next}-release-latest-9.noarch.rpm
fi
...
if [ "$remi" = 'yes' ] && [ ! -e "/etc/yum.repos.d/remi.repo" ]; then
    ...
    sed -i "s/enabled=0/enabled=1/g" /etc/yum.repos.d/remi-php$phpv.repo
fi
