...
if [ ! -z "$(grep ^NAME=\"Red /etc/os-release)" ]; then
    subscription-manager repos --enable codeready-builder-for-rhel-${release}-$(arch)-rpms
    yum install -y https://dl.fedoraproject.org/pub/epel/epel-release-latest-${release}.noarch.rpm
else
    yum config-manager --set-enabled crb
    if [[ $release == 9 ]] ; then yum install -y https://dl.fedoraproject.org/pub/epel/epel{,-next}-release-latest-${release}.noarch.rpm ; fi
    if [[ $release == 10 ]] ; then yum install -y https://dl.fedoraproject.org/pub/epel/epel-release-latest-${release}.noarch.rpm ; fi
fi
check_result $? "Can't install EPEL repository"

# Installing Remi repository
if [ "$remi" = 'yes' ] && [ ! -e "/etc/yum.repos.d/remi.repo" ]; then
    yum install -y http://rpms.remirepo.net/enterprise/remi-release-$release.rpm
...
if [ "$apache" = 'yes' ] && [ "$nginx" = 'no' ] ; then
    ...
    echo "STATS_SYSTEM='webalizer'" >> $VESTA/conf/vesta.conf
fi
if [ "$apache" = 'yes' ] && [ "$nginx"  = 'yes' ] ; then
    echo "WEB_SYSTEM='httpd'" >> $VESTA/conf/vesta.conf
    echo "WEB_RGROUPS='apache'" >> $VESTA/conf/vesta.conf
...
$VESTA/bin/v-delete-web-domain-alias admin $servername www.$servername
